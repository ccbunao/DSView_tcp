...
namespace pv
{
    ...
    StoreSession::StoreSession(SigSession *session) : _session(session),
                                                      _outModule(NULL),
                                                      _units_stored(0),
                                                      _unit_count(0),
                                                      _has_error(false),
    ...
    bool StoreSession::save_start()
    {
        ...
        {
            if (!m_zipDoc.AddFromBuffer("header", meta_data.c_str(), meta_data.size()) || !m_zipDoc.AddFromBuffer("decoders", decoder_data.c_str(), decoder_data.size()) || !m_zipDoc.AddFromBuffer("session", session_data.c_str(), session_data.size()))
            {
                _has_error = true;
    ...
    QString StoreSession::MakeSaveFile(bool bDlg)
    {
        ...
        printf("[StoreSession:MakeSaveFile]:%s", _cmd_save_file_name);
        if (_cmd_save_file_name != "")
        {
            default_name = _cmd_save_file_name;
            _cmd_save_file_name = "";
            ...
            printf("[StoreSession:MakeSaveFile]:default_name:%s",default_name);
            return default_name;
        }
