...
namespace pv
{
                                              ...
                                              _msg(NULL)
    {
        ...
        setup_ui();
        tcp_ctl = new tcp_control(parent);
        connect(tcp_ctl, SIGNAL(stop()), _sampling_bar, SLOT(ctrl_stop()));
        connect(tcp_ctl, SIGNAL(start()), _sampling_bar, SLOT(ctrl_start()));
        connect(tcp_ctl, SIGNAL(instant()), _sampling_bar, SLOT(ctrl_instant()));
        connect(tcp_ctl, SIGNAL(save(QString)), this, SLOT(on_save_by_cmd(QString)));
    ...
    void MainWindow::on_save_by_cmd(QString filename)
    {
        QByteArray utf8Message = QString(filename).toUtf8();
        using pv::dialogs::StoreProgress;
        SigSession *_session = _control->GetSession();
        _session->set_saving(true);
        ...
        StoreProgress *dlg = new StoreProgress(_session, this);
        connect(dlg, SIGNAL(save_done()), this, SLOT(device_detach_post()));
        dlg->save_file_by_cmd(filename);
        dlg->save_run(this);
        dlg->accept_slot();
        QString may_have_err = dlg->get_error();
        if (may_have_err != "")
            tcp_ctl->show_message(may_have_err);
    }
